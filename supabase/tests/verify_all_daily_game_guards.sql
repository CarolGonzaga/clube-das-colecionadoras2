...
begin
  if not exists(select 1 from pg_trigger where tgname='puzzle_game_daily_start_guard' and not tgisinternal)
     ...
     or not exists(select 1 from pg_trigger where tgname='cover_guesser_daily_start_guard' and not tgisinternal)
     or not exists(select 1 from pg_trigger where tgname='cover_guesser_daily_win_guard' and not tgisinternal)
  ...
  if not exists(select 1 from pg_proc where proname='guard_daily_game_start' and prosrc ilike '%puzzle_game_sessions%')
     ...
     or not exists(select 1 from pg_proc where proname='expire_stale_daily_game_sessions' and prosrc ilike '%puzzle_game_sessions%' and prosrc ilike '%cover_guesser_sessions%')
