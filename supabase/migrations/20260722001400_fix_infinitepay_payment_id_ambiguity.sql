-- Avoid PL/pgSQL ambiguity between the local payment identifier and the
-- purchase_payments.provider_payment_id column used by ON CONFLICT.
...
declare
  ...
  order_id_param uuid;
  payment_row_id uuid;
  v_provider_payment_id text := nullif(payment_payload->>'transaction_nsu', '');
...
begin
  if v_provider_payment_id is null or invoice_slug is null or order_nsu is null then
  ...
  insert into public.purchase_payments (
    ...
    order_row.id, 'infinitepay', v_provider_payment_id, invoice_slug,
  ...
  if paid then
    if order_row.status in ('refunded', 'chargeback')
      ...
      insert into public.payment_admin_alerts (
        ...
        order_row.id, payment_row_id, v_provider_payment_id,
      ...
      values (
        ...
        jsonb_build_object('provider', 'infinitepay', 'provider_payment_id', v_provider_payment_id)
      );
    elsif exists (
      select 1
